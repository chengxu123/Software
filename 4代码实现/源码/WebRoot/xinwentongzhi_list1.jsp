...
<html>
 ...
 %>
  ...
  <form name="form2" method="post" action="delall.jsp">
    ...
    <% 
	new CommDAO().delete(request,"xinwentongzhi"); 
    String url = "xinwentongzhi_list1.jsp?1=1"; 
	...
	//out.print(sql);
	ArrayList<HashMap> list = PageManager.getPages(url,1,sql, request ); 
	int i=0;
	...
	%>
        ...
        <td  style="display:none"width="60" align="center"><a href="xinwentongzhi_updt.jsp?id=<%=map.get("id")%>">修改</a>
		 <a onClick="return confirm('确定要删除这条记录吗?')"  href="xinwentongzhi_list1.jsp?scid=<%=map.get("id")%>">删除</a></td>

...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="00FFFF" style="border-collapse:collapse">  
  <tr>
    ...
    <td width="38" bgcolor='#CCFFFF'>手机号</td>
	<td width="38" bgcolor='#CCFFFF'>身高</td>
	<td width="38" bgcolor='#CCFFFF'>体重</td>
	<td width="38" bgcolor='#CCFFFF'>籍贯</td>
	<td width="38" bgcolor='#CCFFFF'>兴趣爱好</td>
	<td width="38" bgcolor='#CCFFFF'>学校</td>
	<td width="38" bgcolor='#CCFFFF'>专业</td>
    <td width="78" bgcolor='#CCFFFF'>身份证</td>
    <td width="94" bgcolor='#CCFFFF'>头像</td>
    <td width="82" bgcolor='#CCFFFF'>地址</td>
    <td width="55" bgcolor='#CCFFFF'>个人历程</td>
   ...
   new CommDAO().delete(request,"yonghuzhuce"); 
	...
	if(request.getParameter("shoujihao")=="" ||request.getParameter("shoujihao")==null ){}else{sql=sql+" and shoujihao like '%"+request.getParameter("shoujihao").trim()+"%'";}
	if(request.getParameter("shengao")=="" ||request.getParameter("shengao")==null ){}else{sql=sql+" and shengao like '%"+request.getParameter("shengao").trim()+"%'";}
	if(request.getParameter("tizhong")=="" ||request.getParameter("tizhong")==null ){}else{sql=sql+" and tizhong like '%"+request.getParameter("tizhong").trim()+"%'";}
	if(request.getParameter("jiguan")=="" ||request.getParameter("jiguan")==null ){}else{sql=sql+" and jiguan like '%"+request.getParameter("jiguan").trim()+"%'";}
	if(request.getParameter("xingquaihao")=="" ||request.getParameter("xingquaihao")==null ){}else{sql=sql+" and xingquaihao like '%"+request.getParameter("xingquaihao").trim()+"%'";}
	if(request.getParameter("xuexiao")=="" ||request.getParameter("xuexiao")==null ){}else{sql=sql+" and xuexiao like '%"+request.getParameter("xuexiao").trim()+"%'";}
	if(request.getParameter("zhuanye")=="" ||request.getParameter("zhuanye")==null ){}else{sql=sql+" and zhuanye like '%"+request.getParameter("zhuanye").trim()+"%'";}
  ...
  <tr>
    ...
    <td><%=map.get("yonghuming") %></td><td><%=map.get("mima") %></td><td><%=map.get("xingming") %></td><td><%=map.get("xingbie") %></td><td><%=map.get("chushengnianyue") %></td><td><%=map.get("QQ") %></td><td><%=map.get("youxiang") %></td><td><%=map.get("shoujihao") %></td><td><%=map.get("shengao") %></td><td><%=map.get("tizhong") %></td><td><%=map.get("jiguan") %></td><td><%=map.get("xingquaihao") %></td><td><%=map.get("xuexiao") %></td><td><%=map.get("zhuanye") %></td><td><%=map.get("shenfenzheng") %></td><td><a href='<%=map.get("touxiang") %>' target='_blank'><img src='<%=map.get("touxiang") %>' width=88 height=99 border=0 /></a></td><td><%=map.get("dizhi") %></td><td><%=map.get("gerenlicheng") %></td>

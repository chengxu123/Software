...
//txixixngdy
    ...
    String url = "shetuanxinxi_list2.jsp?1=1"; 
    String sql =  "select * from shetuanxinxi where shechang='"+request.getSession().getAttribute("username")+"' ";
...
//youzuiping2
 ...
 <td style="display:none"><%=map.get("shouji") %></td>
 <td style="display:none"><%=map.get("baomingfeiyong") %></td>
 
 <td  style="display:none"width='80' align='center'><%=map.get("issh")%></td>
    <td  style="display:none"width="138" align="center"><%=map.get("addtime") %></td>
    <td width="60" align="center"><a href="huodongshenqing_add.jsp?id=<%=map.get("id")%>">活动申请</a> 
	 <!--<a href="shetuanxinxi_list2.jsp?scid=<%=map.get("id") %>" onClick="return confirm('真的要删除？')">删除</a>
	  <a href="shetuanxinxidetail.jsp?id=<%=map.get("id")%>" target="_blank">详细</a>--> </td>
  </tr>
  	<%
  }
   %>
